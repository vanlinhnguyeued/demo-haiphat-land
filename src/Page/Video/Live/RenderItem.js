...
class RenderItem extends Component {
  ...
  render() {
    ...
    return (
      <div className="item_live_video" data-aos="fade-up">
        <div className="item_img_live_video">
          <a
            target={item.Type === 2 ? "_blank" : "_self"}
            href={item.Type === 2 ? item.VideoUrl : "#"}
            onClick={item.Type === 1 ? this.handleItem(item) : () => {}}
          >
            <img src={item.ImgUrl} className="img_live_video" />
          </a>
        </div>
        <div className="item_thongtin_live_video">
          <a
            className="title_item_thongtin_live_video"
            target={item.Type === 2 ? "_blank" : "_self"}
            href={item.Type === 2 ? item.VideoUrl : "#"}
            onClick={item.Type === 1 ? this.handleItem(item) : () => {}}
          >
